...
const SEED_MOCK_DB = {
    ...
    certificates: [
        { id: "CERT-001", employeeId: "EMP-0001635", courseId: "COURSE-102", courseVersionId: "CVERS-102-V31", requirementId: "CREQ-001", evidenceId: "EVID-001", issuedAt: "2026-08-20", expiresAt: "2027-08-20", status: "Active", hashSHA256: "0x8f3a9c148b01c", openBadgesUrl: "https://openbadges.org/v3/claim/8f3a9c", tenantId: "TEN-001" },
        { id: "CERT-002", employeeId: "EMP-0001842", courseId: "COURSE-102", courseVersionId: "CVERS-102-V31", requirementId: "CREQ-001", evidenceId: null, issuedAt: "2025-09-01", expiresAt: "2026-09-01", status: "Expired", hashSHA256: "0x1b2c3d4e5f", openBadgesUrl: "https://openbadges.org/v3/claim/1b2c3d", tenantId: "TEN-001" },
        { id: "CERT-003", employeeId: "EMP-0002104", courseId: "COURSE-102", courseVersionId: "CVERS-102-V31", requirementId: "CREQ-001", evidenceId: null, issuedAt: "2025-10-10", expiresAt: "2026-10-10", status: "Expiring", hashSHA256: "0x9f8e7d6c5b", openBadgesUrl: "https://openbadges.org/v3/claim/9f8e7d", tenantId: "TEN-001" }
    ],
    ...
    notifications: [
        ...
        { id: "NOTIF-003", employeeId: "EMP-0001635", title: "Certificación Aprobada", message: "Tu badge OpenBadges v3.0 ha sido verificado en blockchain.", read: true, createdAt: "2026-08-20T14:35:00Z", tenantId: "TEN-001" }
    ],
    events: [
...
const ProgressService = {
    ...
    updateActivityProgress(command, correlationId = null) {
        ...
        if (!enrollment || enrollment.tenantId !== currentTenant || enrollment.employeeId !== currentUser.id) {
            return createServiceError("E-10", "Inscripción no válida o acceso denegado", correlationId);
        }
    ...
    calculateEnrollmentProgress(enrollmentId, tenantId) {
        ...
        requiredActivities.forEach(activity => {
            ...
            if (progress && Number(progress.progressPct) >= 100) {
...
const CertificationService = {
    getById(id) {
        if (!AuthorizationService.can("learning.certification.read")) return createServiceError("AUTHORIZATION_DENIED", "Permiso denegado");
        const cert = CertificateRepository.getById(id);
        if (!cert) return createServiceError("NOT_FOUND", `Certificado '${id}' no encontrado`);
        return createServiceResult(true, cert);
    },
    getForEmployee(employeeId) {
        if (!AuthorizationService.checkScope("SELF", { employeeId: employeeId }) && !AuthorizationService.checkScope("TEAM", { employeeId: employeeId })) {
            return createServiceError("SCOPE_DENIED", "Acceso denegado a certificados de otro colaborador");
        }
        return createServiceResult(true, CertificateRepository.getByEmployee(employeeId));
    },
    issue(command, correlationId = null) {
        if (!command.employeeId || !command.courseId || !command.courseVersionId || !command.requirementId) {
            return createServiceError("MISSING_REQUIRED_DATA", "Parámetros obligatorios 'employeeId', 'courseId', 'courseVersionId' y 'requirementId' requeridos", correlationId);
        }

        // Dedicated permission check 'learning.certification.issue'
        if (!command.isInternal && !AuthorizationService.can("learning.certification.issue") && !AuthorizationService.can("learning.approval.execute")) {
            AuditRepository.logSecurity("PERMISSION_DENIED", "CertificationService", "issue", "DENIED", "Permiso 'learning.certification.issue' denegado");
            return createServiceError("AUTHORIZATION_DENIED", "Permiso denegado para emitir certificados", correlationId);
        }

        const req = CertificationRequirementRepository.getById(command.requirementId);
        if (!req) {
            return createServiceError("CERTIFICATION_REQUIREMENT_NOT_FOUND", `Requisito de certificación '${command.requirementId}' no encontrado`, correlationId);
        }

        const validityMonths = req.validityMonths;
        if (typeof validityMonths !== "number") {
            return createServiceError("INVALID_STATE", "El requisito de certificación no posee vigencia en meses configurada", correlationId);
        }

        const currentTenant = TenantContext.getCurrentTenantId();

        // Calendar date calculation for exact expiration
        const issueDateStr = (typeof CONFIG !== "undefined" && CONFIG.DEMO_NOW) ? CONFIG.DEMO_NOW.split("T")[0] : new Date().toISOString().split("T")[0];
        const issueDate = new Date(issueDateStr + "T00:00:00Z");
        issueDate.setUTCMonth(issueDate.getUTCMonth() + validityMonths);
        const expiresAtStr = issueDate.toISOString().split("T")[0];

        const certData = {
            id: command.id || ("CERT-" + Date.now()),
            employeeId: command.employeeId,
            courseId: command.courseId,
            courseVersionId: command.courseVersionId,
            ...
            openBadgesUrl: null, // Set to null until real Open Badges v3.0 provider integration
            tenantId: currentTenant
        };

        const created = CertificateRepository.create(certData);
        AuditRepository.logSecurity("SERVICE_OPERATION", "CertificationService", "issue", "ALLOWED", `Certificado '${created.id}' emitido para empleado ${command.employeeId}`, created.id);
    ...
    renew(command, correlationId = null) {
        if (!command.id) return createServiceError("MISSING_REQUIRED_DATA", "Identificador 'id' obligatorio para renovar", correlationId);
        // Dedicated permission check 'learning.certification.renew'
        if (!AuthorizationService.can("learning.certification.renew")) {
            AuditRepository.logSecurity("PERMISSION_DENIED", "CertificationService", "renew", "DENIED", "Permiso 'learning.certification.renew' denegado");
            return createServiceError("AUTHORIZATION_DENIED", "No posees permisos 'learning.certification.renew' para renovar certificaciones", correlationId);
        }
        const cert = CertificateRepository.getById(command.id);
        if (!cert) return createServiceError("NOT_FOUND", `Certificado '${command.id}' no encontrado`, correlationId);

        const currentTenant = TenantContext.getCurrentTenantId();
        if (cert.tenantId && cert.tenantId !== currentTenant) {
            AuditRepository.logSecurity("TENANT_WRITE_DENIED", "CertificationService", "renew", "DENIED", "Cross-tenant certificate renewal rejected");
            return createServiceError("TENANT_WRITE_DENIED", "Violación de aislamiento tenant en renovación", correlationId);
        }

        const issueDateStr = (typeof CONFIG !== "undefined" && CONFIG.DEMO_NOW) ? CONFIG.DEMO_NOW.split("T")[0] : new Date().toISOString().split("T")[0];
        const expireYear = parseInt(issueDateStr.substring(0, 4), 10) + 1;
        const expiresAtStr = expireYear + issueDateStr.substring(4);

        const updated = CertificateRepository.update(command.id, {
            status: "Renewed",
            issuedAt: issueDateStr,
            expiresAt: expiresAtStr
        });

        AuditRepository.logSecurity("SERVICE_OPERATION", "CertificationService", "renew", "ALLOWED", `Certificado ${command.id} renovado`, command.id);

        const evtRes = EventBus.publish({
            eventType: "Learning.CertificateRenewed",
            aggregateId: command.id,
            correlationId: correlationId,
            payload: { id: command.id, employeeId: cert.employeeId },
            producedBy: "CertificationService",
    ...
    getStatus(id) {
        const cert = CertificateRepository.getById(id);
        if (!cert) return createServiceError("NOT_FOUND", `Certificado '${id}' no encontrado`);
        return createServiceResult(true, { status: cert.status, expiresAt: cert.expiresAt });
    }
};
...
function initEventHandlers() {
    ...
    EventBus.subscribe("Learning.EnrollmentCompleted", function onEnrollmentCompleted(evt) {
        const payload = evt.payload || {};
        if (payload.employeeId && payload.courseId && payload.courseVersionId && payload.requirementId) {
...
window.showCourseDetailModal = function(courseId) {
    ...
    if (enr) {
        if (enr.status === "Completed") {
            ctaHtml = '<a href="#/certifications" class="btn btn-obsidian-secondary w-100"><i class="fa-solid fa-award text-emerald me-1"></i> Ver Certificado Digital</a>';
        } else {
        ...
        <div class="row g-3">
            <div class="col-md-9">
                <div class="obsidian-card p-0 overflow-hidden">
                    <div class="bg-black text-center p-5 position-relative" style="min-height: 380px;">
                        <i class="fa-solid fa-circle-play text-cyan display-1 position-absolute top-50 start-50 translate-middle" style="cursor:pointer;" onclick="showToast('xAPI Event Emitted', 'Event: watched_video 75% progress logged', 'success')"></i>
                        <div class="position-absolute bottom-0 start-0 end-0 bg-dark p-2 d-flex align-items-center justify-content-between px-3">
                            <span class="fs-7 text-white"><i class="fa-solid fa-play text-cyan me-2"></i> 14:20 / 22:00 (75%)</span>
                            <span class="badge bg-info">xAPI Telemetry Active</span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="obsidian-card">
                    ...
                    <div class="border-start border-cyan ps-2 mb-2 text-cyan fs-7 fw-semibold">1. Procedimientos Excavación (75%)</div>
                    <div class="ps-2 mb-2 text-muted fs-7">2. Inspección Pre-operacional 360°</div>
                    <div class="ps-2 mb-2 text-muted fs-7">3. Quiz de Certificación Teórica</div>
                </div>
            </div>
        </div>
    `;
...
function renderScreenCertifications() {
    return `
        <div class="obsidian-card">
            <h5 class="fw-bold text-white mb-3"><i class="fa-solid fa-award text-amber me-2"></i> Mis Certificaciones Digitales Verificadas</h5>
            <div class="row g-3">
                <div class="col-md-6">
                    <div class="obsidian-card border-bright bg-navy">
                        <div class="d-flex justify-content-between align-items-start">
                            <div>
                                <span class="status-badge completed mb-2">OpenBadges v3.0 Verified</span>
                                <h6 class="fw-bold text-white">Inducción Normativa SST & Prevención</h6>
                                <div class="text-muted fs-7">Emitido: 20 Aug 2026 | Expira: 20 Aug 2027</div>
                            </div>
                            <i class="fa-solid fa-qrcode display-6 text-cyan"></i>
                        </div>
                        <div class="mt-3 pt-2 border-top border-secondary font-monospace fs-7 text-muted">Hash: 0x8f3a9c148b01c</div>
                    </div>
                </div>
            </div>
        </div>
