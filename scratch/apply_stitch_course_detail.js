...
const startMarker = 'function renderScreenCourseDetail() {';
const endMarker = 'window.renderScreenCourseDetail = renderScreenCourseDetail;';
...
const newFunc = `function renderScreenCourseDetail() {
    ...
    // Units and Activities (Strict Tenant Isolation & Real Ordering)
    ...
    if (userEnrollment && userEnrollment.status === "Completed") {
        ...
        ctaHtml = '<a href="#/player" class="btn btn-cyan text-dark fw-bold px-4 py-2 shadow-sm"><i class="fa-solid fa-play me-2"></i> Continuar Lección (v' + (version.versionNumber || version.version || '2.4') + ')</a>';
        if (userEnrollment.dueDate) {
            dueDateDisplay = userEnrollment.dueDate;
        }
    } else {
    ...
    const versionNumText = version.versionNumber || version.version || "No disponible";
        ...
        unitsContentHtml = \`
            <div class="accordion accordion-flush" id="course-units-accordion">
                \${units.map((u, idx) => {
                    ...
                    const orderNum = u.order < 10 ? '0' + u.order : u.order;
                    const isFirst = idx === 0;
                        ...
                        unitContentHtml = \`
                            <div class="list-group list-group-flush bg-transparent">
                                \${uActivities.map(a => {
                                    ...
                                    const actStatus = a.status || 'Estado no disponible';
...
console.log('Successfully updated renderScreenCourseDetail with Stitch visual alignment!');
